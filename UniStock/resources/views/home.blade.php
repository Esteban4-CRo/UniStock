...
@section('content')
    <div class="row mb-4 align-items-center">
        <div class="col-md-6">
            <h2 class="fw-bold mb-1"><i class="fas fa-chart-pie me-2" style="color: var(--primary);"></i> <span
                    class="animate-soft-blur">Panel de Control Logístico</span></h2>
            <p class="text-muted mb-0">Resumen y estado operativo del almacén de materias primas.</p>
        </div>
        <div class="col-md-6 text-md-end mt-3 mt-md-0">
            <div class="btn-group">
                <a href="{{ route('materias-primas.create') }}" class="btn btn-outline-secondary btn-sm px-3"><i
                        class="fas fa-plus me-1"></i> Materia Prima</a>
                <a href="{{ route('entradas.create') }}" class="btn btn-outline-secondary btn-sm px-3 ms-1"><i
                        class="fas fa-arrow-down me-1"></i> Registrar Entrada</a>
                <a href="{{ route('salidas.create') }}" class="btn btn-outline-secondary btn-sm px-3 ms-1"><i
                        class="fas fa-arrow-up me-1"></i> Registrar Salida</a>
            </div>
        </div>
    </div>
        ...
        <div class="col-sm-6 col-xl">
            <div class="card-base glass p-3 text-center transition-card" style="transition: all 0.3s ease;">
                ...
                <span class="small opacity-75 uppercase fw-semibold text-truncate d-block animate-micro-scale"
                    style="font-size: 0.8rem; letter-spacing: 0.5px;">Materias Primas</span>
            </div>
        </div>
        <div class="col-sm-6 col-xl">
            <div class="card-base glass h-100 p-3 text-center transition-card" style="transition: all 0.3s ease;">
                ...
                <span class="small text-muted uppercase fw-semibold text-truncate d-block animate-micro-scale"
                    style="font-size: 0.8rem; letter-spacing: 0.5px;">Proveedores</span>
            </div>
        </div>
        <div class="col-sm-6 col-xl">
            <div class="card-base glass h-100 p-3 text-center transition-card" style="transition: all 0.3s ease;">
                ...
                <span class="small text-muted uppercase fw-semibold text-truncate d-block animate-micro-scale"
                    style="font-size: 0.8rem; letter-spacing: 0.5px;">Ubicaciones</span>
            </div>
        </div>
        <div class="col-sm-6 col-xl">
            <div class="card-base glass h-100 p-3 text-center transition-card"
                ...
                <h3 class="fw-bold text-success mb-1 mt-2 animate-spring-scale" style="font-size: 2.4rem;">
                    +{{ $totalEntradas }}</h3>
                <span class="small text-muted uppercase fw-semibold text-truncate d-block animate-micro-scale"
                    style="font-size: 0.8rem; letter-spacing: 0.5px;">Entradas Totales</span>
            </div>
        </div>
        <div class="col-sm-6 col-xl">
            <div class="card-base glass h-100 p-3 text-center transition-card"
                ...
                <h3 class="fw-bold text-danger mb-1 mt-2 animate-spring-scale" style="font-size: 2.4rem;">
                    -{{ $totalSalidas }}</h3>
                <span class="small text-muted uppercase fw-semibold text-truncate d-block animate-micro-scale"
                    style="font-size: 0.8rem; letter-spacing: 0.5px;">Salidas Totales</span>
            </div>
        </div>
    </div>
        ...
        <div class="col-12 col-xl-7">
            <div class="card glass shadow-sm border-0 h-100" style="background: transparent;">
                <div class="card-header border-0 py-3 d-flex align-items-center justify-content-between"
                    ...
                    <h5 class="fw-bold mb-0"><i class="fas fa-box me-2" style="color: var(--primary);"></i> Materias Primas
                        Recientes</h5>
                    <a href="{{ route('materias-primas.index') }}"
                        class="btn btn-sm btn-link text-decoration-none fw-bold p-0" style="color: var(--primary);">Ver
                        Inventario completo</a>
                </div>
                                    ...
                                    <tr>
                                        <td class="py-3 ps-4">
                                            ...
                                            <span class="font-monospace text-muted small"
                                                style="font-size: 0.7rem;">{{ $p->codigo }}</span>
                                        </td>
                                        <td class="py-3">
                                            @if($p->ubicacion)
                                        ...
                                        <td class="py-3">
                                            ...
                                            <span class="text-muted font-monospace small"
                                                style="font-size: 0.75rem;">{{ $p->unidad_medida }}</span>
                                        </td>
                                        <td class="py-3 fw-semibold">
                                            ${{ number_format($p->precio, 2) }}
                                        </td>
                                        <td class="py-3 text-end pe-4">
                                            <a href="{{ route('entradas.create') }}?material_prima_id={{ $p->id }}"
                                                class="btn btn-sm border-0 me-1"
                                                style="background: var(--bot-bg); color: var(--text);" title="Entrada">
                                                <i class="fas fa-arrow-down"></i>
                                            </a>
                                            <a href="{{ route('salidas.create') }}?material_prima_id={{ $p->id }}"
                                                class="btn btn-sm border-0"
                                                style="background: var(--bot-bg); color: var(--text);" title="Salida">
        ...
        <div class="col-12 col-xl-5">
            ...
            <div class="card glass shadow-sm border-0 mb-4" style="background: transparent;">
                <div class="card-header border-0 py-3 d-flex align-items-center justify-content-between"
                    ...
                    <a href="{{ route('entradas.index') }}" class="btn btn-sm btn-link text-decoration-none fw-bold p-0"
                        style="color: var(--primary);">Ver Todos</a>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                            ...
                            <tbody>
                                @forelse($entradas as $entrada)
                                    <tr>
                                        <td class="py-3 ps-3">
                                            <div class="fw-bold">
                                                {{ $entrada->materialPrima ? $entrada->materialPrima->nombre : 'Insumo Eliminado' }}
                                            </div>
                                            <small class="text-muted"><i
                                                    class="fas fa-truck me-1"></i>{{ $entrada->proveedor ? $entrada->proveedor->empresa : 'N/A' }}</small>
                                        </td>
            ...
            <div class="card glass shadow-sm border-0" style="background: transparent;">
                <div class="card-header border-0 py-3 d-flex align-items-center justify-content-between"
                    ...
                    <a href="{{ route('salidas.index') }}" class="btn btn-sm btn-link text-decoration-none fw-bold p-0"
                        style="color: var(--primary);">Ver Todos</a>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                            ...
                            <tbody>
                                @forelse($salidas as $salida)
                                    <tr>
                                        <td class="py-3 ps-3">
                                            <div class="fw-bold">
                                                {{ $salida->materialPrima ? $salida->materialPrima->nombre : 'Insumo Merma' }}
                                            </div>
                                            <small class="text-muted"><i
                                                    class="fas fa-location-arrow me-1"></i>{{ $salida->destino }}</small>
                                        </td>
        ...
        <div class="col-lg-8">
            <div class="card glass shadow-sm border-0 h-100" style="background: transparent; border-radius: 16px;">
                <div class="card-header border-0 py-3 pb-0" style="background: transparent;">
                    <h6 class="fw-bold mb-0"><i class="fas fa-server me-2" style="color: var(--primary);"></i> Estado del
                        Almacén</h6>
                </div>
                <div class="card-body">
                        ...
                        <div class="col-md-6 mb-4 mb-md-0">
                            ...
                            <div class="progress" style="height: 10px; background: rgba(0,0,0,0.1); border-radius: 10px;">
                                <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
                                    style="width: 68%; background: var(--primary); border-radius: 10px;" aria-valuenow="68"
                                    aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                            <p class="text-muted small mt-2 mb-0" style="font-size: 0.8rem;">El stock general se encuentra
                                en niveles estables. No hay alertas de sobrecapacidad inminente.</p>
                        </div>
            ...
            <div class="card glass shadow-sm border-0 h-100" style="background: transparent; border-radius: 16px;">
                ...
                <div class="card-body d-flex flex-column gap-2 justify-content-center">
                    <a href="{{ route('reportes.index') }}"
                        class="btn btn-outline-secondary text-start w-100 py-2 d-flex align-items-center"
                        style="border-radius: 10px; border-color: rgba(255,255,255,0.1);">
                        <div class="d-flex align-items-center justify-content-center me-3"
                            style="width: 32px; height: 32px; background: var(--bot-bg); border-radius: 8px;">
