...
@section('content')
    <div class="row mb-4 align-items-center">
        <div class="col-md-6">
            <h2 class="fw-bold text-dark"><i class="fas fa-truck text-dark me-2"></i> <span
                    class="animate-soft-blur">Directorio de Proveedores</span></h2>
                            ...
                            <tr>
                                <td class="py-3 ps-4">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar-circle bg-dark text-white rounded-circle d-flex align-items-center justify-content-center me-3"
                                        ...
                                        <div>
                                            ...
                                            <small class="text-muted">Contacto:
                                                {{ $proveedor->user ? $proveedor->user->name : 'No asignado' }}</small>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-3">
                                ...
                                <td class="py-3">
                                    @if($proveedor->estado_validacion === 'validado')
                                        <span class="badge color-green text-white border border-success"><i
                                                class="fas fa-check-circle me-1"></i> Validado</span>
                                    @elseif($proveedor->estado_validacion === 'rechazado')
                                        <span class="badge color-red text-white border border-danger"><i
                                                class="fas fa-times-circle me-1"></i> Rechazado</span>
                                    @else
                                        <span class="badge color-yellow text-black border border-warning"><i
                                                class="fas fa-clock me-1"></i> Pendiente</span>
                                    @endif
                                </td>
                                <td class="py-3 text-center">
                                    @if($proveedor->hasLocation())
                                        <a href="{{ $proveedor->google_maps_url }}" target="_blank"
                                            class="btn btn-sm btn-outline-secondary py-1 px-2 border-0">
                                ...
                                <td class="py-3 text-end pe-4">
                                    <a href="{{ route('proveedores.show', $proveedor->id) }}"
                                        class="btn btn-sm btn-light border-0 me-1" title="Ver detalle">
                                        <i class="fas fa-eye text-dark"></i>
                                    </a>
                                    @if(Auth::user()->isSuperUsuario() || Auth::user()->isGerente())
                                        @if($proveedor->estado_validacion !== 'validado')
                                            <form action="{{ route('proveedores.validar', $proveedor->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('PUT')
                                                <input type="hidden" name="estado_validacion" value="validado">
                                                <button type="submit"
                                                    class="btn btn-sm btn-success bg-opacity-10 text-success border-0 me-1"
                                                    title="Validar Proveedor"
                                                    onclick="return confirm('¿Marcar proveedor como VALIDADO?');">
                                        ...
                                        @if($proveedor->estado_validacion !== 'rechazado')
                                            <form action="{{ route('proveedores.validar', $proveedor->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    class="btn btn-sm btn-danger bg-opacity-10 text-danger border-0 me-1"
                                                    title="Rechazar Proveedor"
                                                    onclick="return confirm('¿Marcar proveedor como RECHAZADO?');">
                                        ...
                                        <a href="{{ route('proveedores.edit', $proveedor->id) }}"
                                            class="btn btn-sm btn-light border-0 me-1" title="Editar">
                                            <i class="fas fa-edit text-dark"></i>
                                        </a>
                                        <form action="{{ route('proveedores.destroy', $proveedor->id) }}" method="POST"
                                            class="d-inline"
                                            onsubmit="return confirm('¿Está seguro de eliminar este proveedor? Esto borrará su registro permanente.');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    ...
                                    <p class="text-muted mb-0">Los proveedores registrados aparecerán en esta sección para
                                        vincularlos con las entradas de materia prima.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
